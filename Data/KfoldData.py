...
def stratified_kfold_data(whole_data_txt, save_dir, fold_num=5):
    ...
    fatigue_data = whole_data[:, 1].astype(np.float32)
    ...
    for i in range(fold_num):
        np.savetxt(os.path.join(save_dir, '375Data-fold{}-Train.txt'.format(i)), train_fold[i], fmt='%s')
        np.savetxt(os.path.join(save_dir, '375Data-fold{}-Eval.txt'.format(i)), eval_fold[i], fmt='%s')



#===========================================================================================

whole_data_txt = r'C:/Users/admin/Desktop/375Data-sum.txt'
