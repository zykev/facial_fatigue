...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, shortcut_type, stride=1, non_local=False):
        ...
        if non_local:
            layers.append(NLBlockND(in_channels=256, dimension=3))
            #layers.append(block(self.in_planes, planes))
...
def generate_model(model_depth, non_local=False, **kwargs):
    assert model_depth in [10, 18, 34, 50, 101, 152, 200]

    if model_depth == 10:
        model = ResNet(BasicBlock, [1, 1, 1, 1], get_inplanes(), non_local=non_local, **kwargs)
    elif model_depth == 18:
        model = ResNet(BasicBlock, [2, 2, 2, 2], get_inplanes(), non_local=non_local, **kwargs)
    elif model_depth == 34:
