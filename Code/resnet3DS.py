...
class InputChannelerror(RuntimeError):
    def __init__(self):
        print("Inputed channel is not in [8, 16, 32, 64]")
    pass


def get_inplanes(first_channel:int=None):
    if first_channel in [8, 16, 32, 64]:
        if first_channel == 8:
            return [8, 16, 32, 64]
        elif first_channel == 16:
            return [16, 32, 64, 128]
        elif first_channel == 32:
            return [32, 64, 128, 256]
        elif first_channel == 64:
            return [64, 128, 256, 512]
    else:
        raise InputChannelerror
...
if __name__ == "__main__":
    structure = generate_model(10, non_local=True, n_classes=10)
    summary(structure, (3, 32, 224, 224))
